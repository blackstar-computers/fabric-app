...
/// Fit-to-width scale clamped so text never shrinks below readable; wider trees
/// fall back to scrolling rather than squishing further.
pub fn fit_scale(layout: &TreeLayout) -> f32 {
    let (cw, _) = content_size(layout);
    (TARGET_W / cw.max(1.)).clamp(MIN_SCALE, 1.0)
}
...
fn edge_live(node: &TreeNode, fleet_size: i64) -> bool {
    node.relay && node.up && fleet_size > 0
}
...
fn paint_edges(
    ...
    for node in nodes {
        ...
        let mid = (y1 + y2) / 2.;
        let mut b = PathBuilder::stroke(px(if live { 2.0 } else { 1.4 }));
        ...
        if let Ok(path) = b.build() {
            window.paint_path(path, with_alpha(color, if node.up { 1.0 } else { 0.4 }));
        }
    }
}
